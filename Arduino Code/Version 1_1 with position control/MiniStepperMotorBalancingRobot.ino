...
Version 1.1 - increased the loop speed slightly and tuned all the PID parameters better. 
...
Position control: 
  trying to maintain a target position using a third PID controller. does not work very well. 
  Do not use this version if you just want a working balancing robot. 
...
Pid positionPID(POSITION_P, POSITION_I, POSITION_D, POSITION_I_LIMIT);
Buzzer buzzer = Buzzer(BUZZER_PIN); 

boolean balancing = false; 
boolean bateryVoltageCutoff = false; 
...
void loop() {
  // heartbeat LED flashing, if you see this LED flashing, the robot is running it's main loop. 
  if (bateryVoltageCutoff)
    digitalWrite(LED_PIN, millis() % 1000 > 200);
  ...
  if (millis() > 3000 && voltage < MIN_BAT_VOLTAGE) {
    balancing = false;    
    bateryVoltageCutoff = true; 
    buzzer.buzzBlocking(200);
    delay(800); 
  }
  ...
  // balance the robot. 
  //float targetAngle = speedPID.updatePID(targetSpeed, motorSpeed, TARGET_DELTA_TIME); 
  //motorSpeed = -anglePID.updatePID(targetAngle, currentLeanAngle, TARGET_DELTA_TIME);

  targetSpeed = positionPID.updatePID(targetPosition, currentPosition, TARGET_DELTA_TIME); 
  //targetSpeed = 0; 
  
  float targetAngle = speedPID.updatePID(targetSpeed, motorSpeed, TARGET_DELTA_TIME); 
  motorSpeed = -anglePID.updatePID(targetAngle, currentLeanAngle, TARGET_DELTA_TIME);

  currentPosition += motorSpeed * TARGET_DELTA_TIME; 
  ...
  if (!bateryVoltageCutoff && !balancing && millis() > 3000 && abs(currentLeanAngle) < START_ANGLE_ERROR) {
    balancing = true;  
  ...
  if (balancing) {
    ...
    int16_t leftMotorSpeed = (motorSpeed + turningSpeed) * 3600;
    int16_t rightMotorSpeed = (motorSpeed - turningSpeed) * 3600; 
